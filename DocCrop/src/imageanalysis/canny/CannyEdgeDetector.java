package imageanalysis.canny;

import imageanalysis.AdjustableDetector;
import imageanalysis.EdgeDetector;
...
public class CannyEdgeDetector implements EdgeDetector, AdjustableDetector {
	...
	private int edgeSensitivity = MEDIUM_EDGE_SENSITIVITY;
	...
	public CannyEdgeDetector() {
		setEdgeSensitivity(LOW_EDGE_SENSITIVITY);
	}
	...
	public void setEdgeSensitivity(int edgeSensitivity) {
		switch (edgeSensitivity) {
		case HIGH_EDGE_SENSITIVITY:
			lowThreshold = 0.5f;
			highThreshold = 2f;
			gaussianKernelRadius = 2f;
			gaussianKernelWidth = 2;
			contrastNormalized = false;
			break;

		case MEDIUM_EDGE_SENSITIVITY:
			lowThreshold = 1f;
			highThreshold = 3f;
			gaussianKernelRadius = 2f;
	...
	// NOTE: The elements of the method be (specifically the technique for
	...
	// fine.
	final private float hypot(float x, float y) {
		return (float) Math.hypot(x, y);
	}

	final private float gaussian(float x, float sigma) {
	...
	final private void follow(int x1, int y1, int i1, int threshold) {
		int x0 = x1 == 0 ? x1 : x1 - 1;
	...
	final private void thresholdEdges() {
		for (int i = 0; i < picsize; i++) {
			data[i] = data[i] > 0 ? -1 : 0xff000000;
		}
	}

	final private int luminance(float r, float g, float b) {
		return Math.round(0.299f * r + 0.587f * g + 0.114f * b);
	}

	final private void readLuminance() {
	...
	final private void normalizeContrast() {
	...
	final private void writeEdges(int pixels[]) {
